...
func (c *Client) Initialize(filename string) error {
	...
	c.SkipSslCertVerification = c2.SkipSslCertVerification
...
func (c *Client) Authenticate() error {
	...
	*/
	if c.Username != "" && c.Password != "" {
		...
		req := initial{c.Username, c.Password}
		_, err = c.communicate("POST", []string{"authenticate"}, nil, &req, &ans, false)
